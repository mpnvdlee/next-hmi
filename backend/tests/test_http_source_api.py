...
def test_oversized_response_is_abandoned_mid_transfer(proxy_client, monkeypatch):
    """The cap is a memory guard, so it has to bite before the body is buffered:
    the proxy reads one chunk past the cap and drops the connection."""
    chunk = b"x" * 65_536
    available = 64  # 4 MiB if the proxy ever drained it
    pulled = 0
    ...
    _stub_transport(monkeypatch, lambda request: httpx.Response(200, content=body()))

    resp = proxy_client.post("/api/http-request", json={"url": "http://plc.local/big"})

    assert resp.json()["ok"] is False
    assert "exceeds" in resp.json()["error"]
    assert pulled == http_source_api._MAX_BODY_BYTES // len(chunk) + 1
