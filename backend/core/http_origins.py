...
exists in the project the policy permits any http(s) origin. That is a real
widening of the proxy, so it is never silent: it takes an explicit edit in the
editor, which only the device admin can reach.
