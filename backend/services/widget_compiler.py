...
from core.builtin_widgets_manifest import editor_manifest_path
...
# Basename ``publish_builtin_widgets_assets`` will clear and rewrite. The publish
# root arrives from a CLI flag and is emptied before it is refilled, so requiring
# the name bounds a mistyped ``--publish-dir`` to the tree it is meant to own.
PUBLISH_DIR_NAME = "builtin-widgets-js"
...
class _StatusState:
    ...
    as it once was — keeps a built-in-widgets build out of the live project's file.
...
class CompileTarget:
    ...
    handed the same ``out_dir`` or a built-in-widgets build would rewrite the live
...
def compile_target(src_dir: Path | None = None, out_dir: Path | None = None) -> CompileTarget:
    ...
    to the runtime-home widget cache. The built-in-widgets build passes both so
    a build-machine compile never touches runtime-home state. Names are looked up
...
def _content_hash(module_source: str, stylesheet: Path) -> str:
    ...
    Used as the cache-busting build identity in the baked built-in-widgets
    manifest. A
...
async def _compile_discovered_entry(target: CompileTarget, tsx_path: Path, key: str) -> bool:
    ...
    try:
        ...
        with target.lock:
            ...
            # ``generate_builtin_widgets_manifest`` would otherwise re-read and
            # re-scan every compiled module to find it again.
            target.status[key] = {
                "ok": True,
...
async def compile_all(src_dir: Path | None = None, out_dir: Path | None = None) -> bool:
    ...
    # the project's custom widgets and, from the build step, the product's
    # built-in widgets. Calling both "custom widgets" made a 34-widget
    # built-in-widgets build read as if the project had 34 custom widgets.
...
def regenerate_widget_schemas(src_dir: Path | None = None, out_dir: Path | None = None) -> bool:
    """Rewrite ``widget-schemas.json`` from every custom widget under
    ``src_dir``. Returns True on success, False if extraction failed (the
    previous manifest stays on disk in that case).
    ...
    widget still gets one. Only an I/O error is fatal.

    The file describes exactly what this compile produced, which is why its
    ``builtin`` half is always empty: every product widget ships as a baked
    built-in-widgets manifest instead, overlaid by the single reader,
    ``core.validation.structure.load_widget_manifest``, so the product's widgets
    are visible even where no compile has ever run.
    """
...
def _regenerate_widget_schemas(target: CompileTarget) -> bool:
    ...
    try:
        ...
        manifest = extract_schemas(custom_widget_sources=sources)
...
def generate_builtin_widgets_manifest(
    manifest_path: Path,
    src_dir: Path | None = None,
    out_dir: Path | None = None,
) -> bool:
    """Emit the baked built-in-widgets manifest the frontend imports statically.

    Written as two files — ``manifest_path`` and the ``.editor.json`` sibling
    ``core.builtin_widgets_manifest.editor_manifest_path`` names — because the
    frontend pays for them in different places. The runtime half is imported by
    ...
    tools — goes through ``core.builtin_widgets_manifest``, which merges the
    pair back.
    ...
    * ``origin`` — tells the loader to fetch from ``/builtin-widgets-js/`` rather
      than ``/widget-js/``.
    * ``usesRecharts`` — whether the compiled module actually references the
      ...
      .Recharts`` before *every* built-in import, which would drag the chart
    ...
    try:
        ...
        logger.info(
            "Wrote built-in-widgets manifest (%d widget(s)): %s + %s",
            len(rows),
            manifest_path,
            editor_path.name,
        )
        ...
        logger.error("Built-in-widgets manifest generation failed: %s", err)
        return False


def publish_builtin_widgets_assets(
    publish_dir: Path,
    src_dir: Path | None = None,
    out_dir: Path | None = None,
    manifest_path: Path | None = None,
) -> bool:
    """Copy the servable half of a built-in-widgets build into the frontend's
    public tree.
    ...
    a packaged runtime has no ``src/`` and still needs built-in schemas to
    ...
    off ``frontend/public/builtin-widgets-js`` would otherwise delete every
    public asset the app ships; the basename check makes that typo a refusal.
    ...
    try:
        ...
        for source in find_entries(root):
            ...
            if status.get(key, {}).get("ok") is False:
                logger.error(
                    "Skipping built-in publish of %s: it failed to compile in this build", key
                )
                complete = False
                continue
            if not compiled.is_file():
                logger.error("Missing compiled artifact for built-in widget %s", key)
                complete = False
                continue
        ...
        logger.info("Published %d built-in widget(s): %s", published, publish_dir)
        return complete
    except OSError as err:
        logger.error("Built-in publish failed: %s", err)
        return False
...
def _run_once(
    ...
    regenerate the schema manifest. Used by Docker image build, the
    built-in-widgets build step and any CI one-shot.

    ``manifest_path`` additionally emits the frontend-consumable
    built-in-widgets manifest — the baked registry the app imports statically.
    ...
    async def main() -> int:
        ...
        if manifest_path is not None:
            manifest_ok = generate_builtin_widgets_manifest(manifest_path, src_dir, out_dir)
        publish_ok = True
        if publish_dir is not None and manifest_ok:
            publish_ok = publish_builtin_widgets_assets(
            ...
            logger.error("Skipping built-in publish: manifest generation failed")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--manifest",
        type=Path,
        help="also emit the frontend-consumable built-in-widgets manifest at this path",
    )
    parser.add_argument(
        "--publish-dir",
        type=Path,
