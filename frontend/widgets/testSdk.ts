...
/**
 * Bind the runtime SDK for a built-in widget unit test.
 *
 * A built-in widget source imports nothing and references every helper as a
 * free identifier; the compiler resolves those by prepending a
 ...
 * which silently defeats mocking in unrelated suites. Import it here, per
 * built-in widget test file, and only those files pay for it:
 *
 *     import '../../testSdk';
 *
 * Loading a built-in module by URL is handled separately, by the
 * `resolve.alias['@shared/utils/widgetModuleLoader']` entry in vitest.config.ts
 * — an app test that renders a built-in widget needs that and not this.
 */
installNextHmiSdk();
