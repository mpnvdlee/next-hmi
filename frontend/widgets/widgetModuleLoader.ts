 ...
 *
 * The registry fetches `/builtin-widgets-js/<Group>/<Name>/index.js` (plus a
 * `?t=` cache-buster) from a served build. jsdom has no such server, so
 * resolve the same key from the widget's TSX source instead.
 *
 ...
 * that never render a built-in widget.
 */
...
export function loadWidgetModule(url: string): Promise<WidgetModule> {
  const key = new URL(url, 'http://localhost').pathname.replace(/^.*\/builtin-widgets-js\//, '');
  const load = sources[`./${key.replace(/\.js$/, '.tsx')}`];
  if (!load) return Promise.reject(new Error(`no built-in widget source for "${url}"`));
  return load();
}
