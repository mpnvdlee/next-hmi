...
export function LengthField({
  ...
  // A unit this field's list does not name that the row was on before it was
  // written over — see `offeredUnits`. Held across the field's own commits
  // only: a value from anywhere else (an undo, another widget selected)
  // replaces what this row was editing, and takes the memory with it.
  const displacedUnit = useRef<string | null>(null);
  const selfCommit = useRef(false);

  useEffect(() => {
    setPendingUnit(null);
    if (!selfCommit.current) displacedUnit.current = null;
    selfCommit.current = false;
  }, [value]);

  function commit(num: string, nextUnit: string) {
    if (unit && !units.includes(unit)) displacedUnit.current = unit;
    selfCommit.current = true;
    if (KEYWORD_UNITS.includes(nextUnit)) {
      onChange(nextUnit);
      return;
    }
    if (num === '') {
    ...
    onChange(nextUnit ? `${num}${nextUnit}` : num);
  }
  ...
  // A unit outside this field's list joins its rotation rather than being
  // overwritten: `nextUnit` reads `indexOf` === -1 as "start over", so one
  // click on a project's stored `40vh` rewrote it to `px` with no way back.
  // Keeping it in the list is also what puts it in the dropdown.
  const foreignUnit = unit && !units.includes(unit) ? unit : displacedUnit.current;
  const offeredUnits = foreignUnit ? [foreignUnit, ...units] : units;

  function cycleUnit() {
    const next = nextUnit(unit, offeredUnits);
