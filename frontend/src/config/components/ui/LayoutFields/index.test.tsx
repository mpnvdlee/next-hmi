...
describe('LayoutFields size mode', () => {
  ...
  // Once a literal mode lands on either axis, the node has left the pre-mode
  // shape and the runtime defaults the *other* axis's still-unset mode to Hug
  // (`effectiveSizeMode` in layoutUtils.ts) — Hug suppresses that axis's own
  // length. The panel has to read the same effective mode, or it keeps
  // offering a Height row the runtime has already stopped reading.
  it('disables the other axis once a literal mode makes it the effective Hug', () => {
    setup({ widthMode: 'fixed', width: '200px' }); // heightMode left unset

    expect(screen.getByLabelText('Height (set by Hug)')).toBeDisabled();
  });

  // The mirror case: with neither axis carrying a literal mode, a bare stored
  // `width` is still the pre-mode shape (see `isPreModeShape`), so Width's own
  // unset mode has nothing pushing it to Hug and its length stays live.
  it('keeps an axis editable in the pre-mode shape, with no literal mode anywhere', () => {
    setup({ width: '200px' });
  ...
  // Unlike a `$var`/`$switch`-unreadable mode, a genuinely unset one is not a
  // hedge once the node has left the pre-mode shape — `effectiveSizeMode`
  // reads it as the same firm Hug the runtime falls back to the moment the
  // other axis owns a literal mode, so the length goes (this is the panel/
  // runtime disagreement `effectiveSizeMode` closes). `grow`'s own
  // reachability is a separate read that still treats an unset axis as
  // unresolved, so its row stays regardless.
  it('still offers the grow weight for a genuinely unset mode, even once its length derives to Hug', () => {
