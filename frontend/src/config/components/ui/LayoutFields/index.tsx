...
export function LayoutFields({
   ...
   *
   * A mode that cannot be read here — a `$var`, a `$switch`, a mixed selection,
   * or genuinely unset — shows the length: it may be the live key once the mode
   * resolves, so it stays reachable and `sizeModePatch` does not clear it.
   */
  function sizeValueRow(axis: SizeAxis, f: LayoutFieldDef): ReactNode {
    const literalMode = literalModeOf(axis);

    // 'fixed', an unset mode (an axis with no mode emits none of the
    // `--w-*`/`--h-*` flow-intent vars, so its own `width`/`height` sizes it
    // same as under a literal Fixed), or a mode no read can resolve.
    if (reachableSizeKeys(literalMode).length) return renderRow(f, schemaFor(f));

    // Only the two deriving modes reach here — every other mode, unreadable
    // ones included, keeps its length reachable above.
    const derivedBy = literalMode === 'fill' ? 'Fill' : 'Hug';
    return (
