  ...
  return (
    <>
      ...
      {open && (
        <PeerTransferModal
          direction="push"
          source={{ id, name: name ?? 'this project' }}
          onClose={() => setOpen(false)}
          // A push leaves this device's project unchanged, so there is
          // nothing to reload here — closing on completion (as this used to)
          // would yank the modal away before the Complete phase and the
          // result ever render. Matches the manager mount, which also leaves
          // it open.
          onTransferred={() => {}}
        />
      )}
