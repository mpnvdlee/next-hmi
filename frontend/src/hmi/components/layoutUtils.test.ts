...
describe('selfLayoutStyle — size-mode flow intent', () => {
  ...
  it('renders nothing by a grow no axis has set to Fill', () => {
    // The Fill-weight row shows while an axis is merely unset — a hedge, as
    // docs/user/layout.md puts it, that "never actually renders the weight
    // until you set it to Fill yourself". A literal flexGrow here broke that
    // promise: both mode rows read "Hug (default)" and the widget grew anyway.
    const style = selfLayoutStyle({ grow: 2, minWidth: '0' });
    expect(style).not.toHaveProperty('flexGrow');
    expect(style).not.toHaveProperty('--w-grow');
    expect(style).not.toHaveProperty('--h-grow');
    expect(style).toMatchObject({ minWidth: '0', '--w-shrink': '0', '--h-shrink': '0' });
  });

  it('still passes a pre-size-mode grow through beside a raw width', () => {
    // A node the migration could read no axis for keeps its raw `width` (or
    // `height`), and that is what marks the pre-mode shape — Hug would
    // suppress the very length still sizing it, so its `grow` rides along.
    const style = selfLayoutStyle({ grow: 2, width: '200px' });
    expect(style).toEqual({ flexGrow: 2, width: '200px' });
  });
