...
Four situations, none of which apply to a plant that just runs the thing:

- **You redistribute a modified build** — a machine builder embedding NEXT HMI in shipped equipment, an integrator delivering a modified build to clients, a SaaS vendor hosting one. Priced per device, once, perpetual; one server is one device, and the browser screens on it are never counted.
